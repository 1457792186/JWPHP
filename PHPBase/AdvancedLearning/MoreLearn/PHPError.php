...
//不过，简单地终止脚本并不总是恰当的方式。研究一下用于处理错误的备选的 PHP 函数


//****************************************创建自定义错误处理器
//创建一个自定义的错误处理器非常简单。很简单地创建了一个专用函数，可以在 PHP 中发生错误时调用该函数。
...
//****************************************Set Error Handler
//PHP 的默认错误处理程序是内建的错误处理程序。打算把上面的函数改造为脚本运行期间的默认错误处理程序。
//可以修改错误处理程序，使其仅应用到某些错误，这样脚本就可以不同的方式来处理不同的错误。
//不过，在本例中，打算针对所有错误来使用自定义错误处理程序
...
//如果发生了 E_USER_WARNING，将使用自定义错误处理程序并结束脚本
//error handler function
function customError($errno, $errstr)
{
    echo "<b>Error:</b> [$errno] $errstr<br />";
