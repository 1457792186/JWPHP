...
//正因为它是旧类的副本，因此它从旧类继承了属性和方法，可以使用 exception 类的方法，比如 getLine() 、 getFile() 以及 getMessage()。
...
//****************************************重新抛出异常
//有时，当异常被抛出时，也许希望以不同于标准的方式对它进行处理。可以在一个 "catch" 代码块中再次抛出异常。
//脚本应该对用户隐藏系统错误。对程序员来说，系统错误也许很重要，但是用户对它们并不感兴趣。为了让用户更容易使用，可以再次抛出带有对用户比较友好的消息的异常
class customException extends Exception
{
    public function errorMessage()
    {
